...
def create_broker_instance(config: BrokerConfig, exchange_name: str) -> BrokerClient:
    ...
    if config.url.startswith("amqp"):
        return RabbitMQBroker(config, exchange_name)
    elif config.url.startswith("redis"):
        return RedisBroker(config, exchange_name)
    else:
